...
st.caption(
    "A tool that can be updated every year or every month, so we can keep using it for a long time. "
    "Also, for each department, it can provide business suggestions that are practical to implement, "
    "to reduce risks and improve ESG performance."
)
...
# ── display KPIs ─────────────────────────────────────────────────────────
st.markdown("---")
st.subheader(f"National View — {selected_year} ({scenario_label})")

st.markdown(
    "What's the overall picture for the country? "
)
...
st.caption(
    "💡 *Answer questions like: Are we facing a national supply risk? "
    "Is cost / CO₂ trending up or down?*"
)
...
st.markdown(
    "Where is the risk/opportunity concentrated?"
)
